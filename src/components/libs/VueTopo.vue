 ...
 * @LastEditTime: 2018-11-27 10:57:15
 -->
<template>
<div class="zr-column" ref="topoRef" id="vueTopo">
  <div class="topoComponent" @mousedown.stop="showPopover.show = false">
    ...
    <div class="svgMain" >
      ...
      <div :id="'topoId'+topoId" class="topoWrap" ref="topoWrap">
        ...
        <svg class="topoSvg" id="topoSvg" :style="'background:url(' + topoData.bgUrl + ') 0% 0% / ' + svgAttr.width + 'px ' + svgAttr.height + 'px no-repeat;'"
          :width="svgAttr.width" 
          :height="svgAttr.height"  
          @mousedown.stop="mousedownTopoSvg($event)"
          @mouseover.stop="isMouseDown&&editable?dblclickNode():false"
          ...
          <g>                        
            <g
              ...
              @mousedown.stop="isMouseDown = true,dragSvgNode(key,$event)"
              ...
              <image class="nodeImg" v-if="ele.classType == 'T1'" :xlink:href="ele.image" :x="ele.width - 18" :y="3" height="15px" width="15px"/> -->

              <!-- <el-tooltip class="item" effect="dark" :content="ele.name" placement="top">
                <image class="nodeImg" v-if="ele.classType == 'T2'" :xlink:href="ele.icon" :x="7" :y="7" height="36px" width="36px" :xlink:title="ele.name"/>
              </el-tooltip> -->
              <text  v-if="ele.classType == 'T2'" class="nodeName" x="10" y="60">{{ele.deviceAccount?ele.deviceAccount.deviceName:ele.name}}</text>
    ...
    <div v-if="shapebarMoveNode.isShow&&editable" style="width:90px" class="moveNode nodeMoveCss" :style="{ left:shapebarMoveNode.left + 'px', top: shapebarMoveNode.top + 'px' }">
      <!-- <div class="shapeIcon">
        <img class="shapeIconImg" :src="shapebarMoveNode.image"/>
      </div> -->
      <div class="shapeName">{{shapebarMoveNode.name}}</div>
    </div>
  </div>

  <div class="zr-row-center m-t-20 m-b-20">
      <el-button type="primary" class="w-120-px" @click="saveTopoJson()" v-show="editable">保存</el-button>
      <!-- <el-button type="primary" class="w-120-px" @click="saveTopoImage()">保存图片</el-button> -->
  </div>
  <!-- 详情弹框 -->
  <el-popover v-model="showPopover.show" v-if="selectedNode.realTimeValue" placement="top-start" ref="popover" style="position:absolute;" :style="{'top':showPopover.top - (popoverMoveTop?266:0) + 'px','left':showPopover.left - (popoverMoveLeft?266:0) + 'px'}">
    <div class="zr-row-scatter">
      <div class="w-100-px">bnm</div>
      <div>{{selectedNode.realTimeValue.bnm}}</div>
    </div>
    <div class="zr-row-scatter zr_align-center">
      <div class="w-100-px">tds</div>
      <div>{{selectedNode.realTimeValue.tds}}</div>
    </div>
    <div class="zr-row-scatter zr_align-center">
      <div class="w-100-px">tnm</div>
      <div>{{selectedNode.realTimeValue.tnm}}</div>
    </div>
    <div class="zr-row-scatter zr_align-center">
      <div class="w-100-px">val</div>
      <div>{{selectedNode.realTimeValue.val}}</div>
    </div>
    <div class="zr-row-scatter zr_align-center">
      <div class="w-100-px">vq</div>
      <div>{{selectedNode.realTimeValue.vq}}</div>
    </div>
    <div class="zr-row-scatter zr_align-center">
      <div class="w-100-px">vt</div>
      <div>{{selectedNode.realTimeValue.vt}}</div>
    </div>
  </el-popover>
</div>
</template>
...
import vTopoAttrPanel from './vTopoAttrPanel'
import saveSvgAsPng from 'static/js/saveSvgAsPng.js'
...
export default {
  ...
  data () {
    return {
      ...
      //详情弹框的左边，上边是否偏移
      popoverMoveLeft: false,
      popoverMoveTop: false,
      keyFormRules:{
      key:[
  ...
  methods:{
    ...
    //拖拽shapeBar中的node
    dragShapeNode(node,event){
      const _this = this
      this.showPopover.show = false
      let NODE = node
      let toolbarName =NODE.deviceName
      let toolbarIcon = NODE.image
      ...
      document.onmousemove = (event) =>{
        ...
        // this.$root.$children[0].$children[0].$refs.contentBox 获取页面主题元素与浏览器的宽高   8是鼠标和图片的偏差 34是面包屑高度
        var obj = this.$root.$children[0].$children[0].$refs.contentBox
        this.shapebarMoveNode.left = mouseX - obj.offsetLeft - 8 + obj.scrollLeft  // 鼠标位置 + 文档滚动的距离
        this.shapebarMoveNode.top =  mouseY - obj.offsetTop - 34 + obj.scrollTop
        this.shapebarMoveNode.name = toolbarName
        this.shapebarMoveNode.image = toolbarIcon
      ...
      document.onmouseup = (event)=>{
        ...
        document.onmouseup = null
         // 判断鼠标在svg区域
        if(isContainSvgArea){
            let TOPODATA = _this.topoData
            let type = NODE.type
            let name = NODE.deviceName
            NODE.num ++ 
            let id = GenNonDuplicateID(5)
            let nodeEndX = _this.marker.ymarkerX
            let nodeEndY = _this.marker.xmarkerY
            let svgNode ={
               name,
               type,
               id:id,
               ...
               containNodes:[],
               attrs:[],
               deviceId: NODE.deviceNo
            }
            _this.marker.isMarkerShow = false    //标尺取消显示
            _this.topoData.nodes.push(svgNode)   //创建一个svg Node    
            //计算是否与某个节点重叠
            for(let i =( TOPODATA.nodes.length - 1); i >= 0; i-- ){
              ...
              if(node.x <= nodeEndX && nodeEndX <= (node.x + node.width) && nodeEndY >= node.y && node.y + node.height >= nodeEndY && node.id != id){
                let canBeContain =  _this.canConnectorTo(NODE.type,node.type,'Contain')  //判断是否能被包含在目标元素中
                if(canBeContain){
                  let connectorId = _this.GenNonDuplicateID(3)
                  let connector={
                    id:connectorId,
                    type:'Contain',
                    sourceNode:{
                  ...
                  _this.refreshRowAndOuterNode(svgNode)  //刷新并列节点位置和父节点宽高 
                  _this.refreshConnectorsData()  
                  break
                }          
              }
            }
    ...
    dragSvgNode(key,event){
      var obj = this.$root.$children[0].$children[0].$refs.contentBox
      if(obj) {
        this.showPopover.left = event.clientX - obj.offsetLeft + obj.scrollLeft
        this.showPopover.top = event.clientY - obj.offsetTop + obj.scrollTop - (this.isIndex?0:34)
      }
      let NODE = this.topoData.nodes[key]
      ...
      this.$nextTick(() => {
        ...
        if(w - event.clientX < 300) this.popoverMoveLeft = true
        else this.popoverMoveLeft = false
        if(h - event.clientY < 300) this.popoverMoveTop = true
        else this.popoverMoveTop = false
      })
...
　　　　};
    ...
    storeCurnodeStartPosition(CURNODE,startNodePosition){
      ...
      if(containNodes&&containNodes.length){
...
　　　 document.onmouseup = null 
    ...
    deleteCurnodeAndChildnodes(CURNODE){      
      this.deleteCurNodeContainConnector(CURNODE)
      if(CURNODE.containNodes&&CURNODE.containNodes.length){        
    ...
    //保存topo的json数据
    saveTopoJson(){
      axiosHttp.post('/manage/topoNode/saveNodes',Object.assign(this.topoData,{floorId:1,bgUrl:this.bgUrl,width:this.topoW,height:this.topoH}))
    },
    saveTopoImage(){
      let maxW = 0
      let maxH = 0
    ...
    initTopoWH(){
      this.$nextTick(()=>{
        ...
        this.topoW = $(ele).width()
        // let topoH = h - 50 - 34 - 20
        this.topoH = this.topoW/4 * 3
        this.marker.xmarkerX = this.topoW
        this.marker.ymarkerY = this.topoH
        this.svgAttr.width = this.topoW
        this.svgAttr.height = this.topoH
        this.svgAttr.minW = this.topoW
        this.svgAttr.minH = this.topoH
...
.topoComponent{width:100%;box-sizing: border-box;position: relative;background-color: #fff;height:100%;display:flex;flex-direction: column;}
/*svgHead工具栏*/
.svgHead{width: 100%;height:40px;box-sizing: border-box;padding:0 20px;display:flex;justify-content: space-between;align-items:center;background:@theme-color;border:solid @border-color;border-width:1px 1px 0;box-shadow:inset 0 1px 0 0 #fff;
  .svgHeadItemLst{display:flex;
    .svgHeadItem{padding:5px 10px;border:1px solid @border-color;cursor:pointer;list-style:none;border-left-width: 0;
      ...
      .svgHeadItemImg{background: url('../../assets/icons.png');width:16px;height:16px;background-size:479px 16px;
...
.nodesG{-webkit-user-select:none;user-select:none;-moz-select:none;-ms-select:none;-o-select:none;
  ...
  .nodeName{font-size:12px;fill:#333;-webkit-user-select:none;user-select:none;}
