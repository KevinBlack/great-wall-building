<template>
    <div  class="wrap">
        ...
        <div class="main_content ">
            <div v-for="data in tableData"  style="margin-top: 10px">

                <video width="320" height="240" style="float: left;margin-left: 30px" controls="controls" autoplay="autoplay">
                    <source src="http://10.20.146.131:6713/mag/hls/0b2e85c0b58945e19b771542dd7dbc2a/0/live.m3u8" type="video/mp4">
                    Your browser does not support the video tag.
                </video>
                <div style="">
                    <el-button type="primary" @click="deleteDevice(data)">删除</el-button>
                </div>
        ...
        <!-- 树节点编辑 -->
        <el-dialog   title="添加摄像头" :visible.sync="editTreeNodeVisible" >
            <el-form :model="treeNode" :rules="rulesTreeNode" ref="treeNode22" class="demo-form-inline" label-width="80px">
                <el-form-item label="" prop="houseName">
                    ...
                    <el-select v-model="deviceId" placeholder="选择摄像头" clearable>
                        <el-option v-for="item in cameraList" :key = item.id :label="item.deviceName" :value="item.id"></el-option>
                    </el-select>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button type="primary" @click="editTreeNode()">确 定</el-button>
                <el-button @click="editTreeNodeVisible = false">取 消</el-button>
            </span>
        </el-dialog>
        <div id="rMenu">
            <ul>
                <li id="m_add" @click="handleAddNode()" >添加摄像头</li>
            </ul>
        </div>
        </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteDevice(data){
                console.info(data)
                axiosHttp.post('/manage/monitor/deleteDevice', {
                    // 楼层Id
                    deviceId: data.id,
            ...
            handleAddNode() {
                ...
                this.houseId = this.rightClickNode.id;
            ...
            // 保存摄像机
            editTreeNode() {
                axiosHttp.post('/manage/monitor/save', {
                    // 楼层Id
                    houseId: this.houseId,
                    // 设备Id
            ...
            loadTable() {
                ...
                axiosHttp.post('/manage/monitor/queryByHouseId', {
                    // 楼层Id
                    houseId: this.houseId
                }).then(res => {
            ...
            //  获取全部摄像头
            getCameraList(){
                this.$http('post', '/manage/device/getCameraList', {
                    // name: this.kw,
                    // customerInfoId: this.customerInfoId,
                    // page: this.pageNum2,
                    // size: this.pageSize2,
                }).then(res => {
                    this.cameraList=res.data.body
                    /*this.tableData2 = res.data.body.content;
                    this.totolNum2 = Number(res.data.body.totalElements);*/
                });
            }
