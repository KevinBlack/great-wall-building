<template>
    <div class="wrap">
        <div class="subContent">
            <div class="tableWrap">
                ...
                <el-form :inline="true" :model="searchForm" class="demo-form-inline zr-row-scatter">
                    <div class="zr-row" style="margin-right:20px">
                        <el-button type="primary" @click="editServe()" class="btnClass">填报</el-button>
                       ...
                       <!-- <el-button type="primary" @click="toWriteJudge()" class="btnClass">评价</el-button> -->
                    </div>
                    <div class="zr-row">
                        <el-form-item label="">
                            <!-- 关键字 -->
                ...
                <el-table ref="table" stripe :data="tableData" style="width: 100%" @selection-change="onSelect">
                    ...
                    <!-- 服务单号 -->
                    <el-table-column :show-overflow-tooltip="true" prop="sysAlarmNo" align="center" label="服务单号">
                    ...
                    <el-table-column :show-overflow-tooltip="true" label="操作" align="center" width="180">
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" size="mini" :disabled="scope.row.serverState !=3?true:false" @click="toWriteJudge(scope.$index, scope.row)">评价</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <!--分页-->
            ...
            <!-- 填报-->
            <el-dialog   title="填报服务" :visible.sync="addEditDialogVisible" :before-close="close" width="900px">
            		...
            		<el-row>
            		    ...
            		    <el-col :span="24">
            		        <el-form-item label="问题描述" prop="workWt">
            		            <el-input v-model="editform.workWt" type="textarea" :autosize="{minRows:5}"  maxlength="100"></el-input>
            		        </el-form-item>
            		    </el-col>
            		</el-row>
            		<el-row>
            		    ...
            		    <el-col :span="24">
            		        <el-form-item label="备注" prop="remark">
            		            <el-input v-model="editform.remark" type="textarea" :autosize="{minRows:5}"  maxlength="100"></el-input>
            		        </el-form-item>
            		    </el-col>
            		</el-row>
                </el-form>
            ...
            <!--服务详情弹出窗-->
            <el-dialog   :close-on-click-modal="false"  title="服务详情" :visible.sync="serveDialogVisible" width="900px">
                ...
                <el-form :model="editform" ref="editform"  label-width="120px">
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item label="服务类型" prop="fuType">
                                <!-- <el-input v-model="editform.fuType" disabled></el-input> -->
                    ...
                    <el-row>
                        ...
                        <el-col :span="12">
                            <el-form-item label="评价日期" prop="appraiseTime">
                                <el-date-picker v-model="editform.appraiseTime" type="date" value-format="timestamp"
                                                format="yyyy-MM-dd"  disabled>
                                </el-date-picker>
                            </el-form-item>
                        </el-col>
                ...
                <!-- 已取消服务 -->
                <el-form label-width="120px" v-show="isShowCancleRy">
            ...
            <el-dialog   :close-on-click-modal="false"  title="评价" :visible.sync="writeDialogVisible" width="900px">
                ...
                <!-- 评价信息 -->
                <el-form :model="editformAppraise" :rules="rules" ref="editformAppraise"  label-width="120px" >
                    <span style="font-weight: 600;display: inline-block;margin: 20px 0;">评价信息</span>
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="评价" prop="appraise">
    ...
    export default {
        ...
        data:  function() {
            ...
            return {
                ...
                serverAddDialogVisible:false,//创建
                serveVisible:false,//预约服务 服务工单详情
        ...
        methods: {
                ...
                this.$refs.editform.validate((valid) => {
                    if(valid) {
                        ...
                        if(this.editform.tsTime != undefined) {
                            serveTime = formatDate(new Date(Number(this.editform.tsTime)), "yyyy-MM-dd hh:mm:ss");
                        }
            ...
            //获取所属企业数组
            getCompanys(){
            	this.$httpPost('/message/workManagement/getQieName').then(res => {
            		if(res.data.msg==="success"){
                        console.log(res.data.body.id);
            			this.khcompanyId=res.data.body.id;
                        this.editform = {
                            companyName:res.data.body.name,
                            companyId:res.data.body.id
                        };
            		}
            	},error => {
            	})
            ...
            //取消服务 确定
            cancleRow(){
                  axiosHttp.post('/message/bespeak/QxManagement', {
            ...
            toWriteJudge(index,row){
                ...
                if (row) {
                        ...
                        this.editformAppraise.appraiseTime = currentTime;
                        this.editformAppraise.appraiseName = localStorage.getItem('csmpLoginName');
                        this.editformAppraise.appraiseNum = localStorage.getItem('csmpLoginUserName');
                ...
                this.$refs.editformAppraise.validate((valid) => {
                    if(valid) {
                        ...
                        if(this.editformAppraise.appraiseTime != undefined) {
                            serveTime = formatDate(new Date(Number(this.editformAppraise.appraiseTime)), "yyyy-MM-dd");
                        }
                   ...
                   if(row.workType == 1){//预约服务详情
                       ...
                       axiosHttp.post('/message/workManagement/get', {
                           ...
                           if(res.result.cancelRy == null){//不展示取消原因
                               this.isShowCancleRy = false;
                           }else{
                               this.isShowCancleRy = true;
                           }
            ...
            close() {
                ...
                this.addEditDialogVisible = false;//填报
