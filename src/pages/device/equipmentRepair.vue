        ...
        <div class="subContent">
            <div class="tableWrap">
                    ...
                    <div class="zr-row">
                        ...
                        <el-form-item label="" >
                         ...
                         <el-form-item label="">
                            ...
                            <el-select v-model="searchForm.repairType" placeholder="维修类别" clearable>
                ...
                <el-table ref="table" stripe :data="tableData" style="width: 100%" @selection-change="onSelect">
                    ...
                    <!-- 设备编号 -->
                    <el-table-column :show-overflow-tooltip="true" prop="repairNo" label="维修编号" align="center">
                        <template slot-scope="scope">
                            <el-button type="text" size="smart" @click="openRepair(scope.$index, scope.row)">{{scope.row.repairNo}}</el-button>
                        </template>
                    </el-table-column>
                    <!-- 维修设备 -->
                    <el-table-column :show-overflow-tooltip="true" prop="deviceName" label="维修设备" align="center"></el-table-column>
                    <el-table-column :show-overflow-tooltip="true" prop="deviceId" label="设备编号" align="center"></el-table-column>
                    <!-- 维修类别 1自维 2外维 -->
                    <el-table-column :show-overflow-tooltip="true" prop="repairType" label="维修类别" :formatter="repairType" align="center"></el-table-column>

                    <!-- 故障类别 1机械故障 2突发事件 3线路故障 4签报 5内部维修 6日常维修 7保修 -->
                    <el-table-column :show-overflow-tooltip="true" prop="faultClass" label="故障类别" :formatter="faultClass" align="center"></el-table-column>
                    <!-- 维修开始日期 -->
                    <el-table-column :show-overflow-tooltip="true" prop="startTime" label="维修开始日期" :formatter="startTime" width="120" align="center"></el-table-column>
                    <!-- 维修结束日期 -->
                    <el-table-column :show-overflow-tooltip="true" prop="endTime" label="维修结束日期" :formatter="endTime" width="120" align="center"></el-table-column>
                    <!-- 维修结束日期 -->
                    ...
                    <!-- 维修负责人 -->
                    <el-table-column :show-overflow-tooltip="true" prop="repairUser" label="维修人员" width="100" align="center"></el-table-column>
                    <!-- 数据来源 1自动生成 2手动添加 -->
                    ...
                    <el-table-column :show-overflow-tooltip="true" label="操作" width="180" align="center">
                        <template slot-scope="scope">
                            <el-button type="text" size="mini" @click="toEdit(scope.$index, scope.row)" :disabled="scope.row.alarmId!=null">编辑</el-button>
                            <el-button type="text" size="mini" @click="toDelete(scope.$index, scope.row)" :disabled="scope.row.alarmId!=null">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <!--分页-->
                    ...
                    <div class="p-10 f-w-700 m-b-10">维修信息</div>
                        ...
                        <el-col :span="24">
                            <el-form-item label="材料使用情况" >
                                <!-- 材料使用情况 -->
                                <el-input v-model="editform.material"  :autosize="{minRows:5}" :disabled="isDetail" maxlength="100"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="24">
    ...
    export default {
        ...
        methods: {
            ...
            arr.forEach(element => {
                ...
                transObj.repairNo = element.repairNo
                transObj.deviceName = element.deviceName
                transObj.deviceId = element.deviceId
                transObj.repairType = (element.repairType === '1'?'自维':'外维')
                transObj.faultClass = (this.faultClassCopy(element.faultClass))
                ...
                transObj.repairUser = element.repairUser
                transArray.push(transObj)
            });
            //列标题，逗号隔开，每一个逗号就是隔开一个单元格
            let str = `序号,维修编号,维修设备,设备编号,维修类别,故障类别,维修开始日期,维修结束日期,维修人员\n`;
            ...
            exportExcel() {
                if (this.checkedArr.length === 0) {
                    this.$confirm('确认要导出所有数据吗?', '提示', {
                        ...
                        axiosHttp.post('/manage/equipmentRepair/find',{
                        page: '',
                        size: ''
                        }).then(res => {
            ...
            search(type) {
                const param = {
                    ...
                    repairType: this.searchForm.repairType,
            ...
            faultClassCopy: function(faultClass, column, cellValue) {
                switch (faultClass){
                    case "1":
                        return "机械故障";
                    case "2":
